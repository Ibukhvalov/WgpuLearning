...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>, @builtin(local_invocation_id) local_id: vec3<u32>) {    
    ...
    for(var k: u32 = 0; k < matrix_size; k += TILE_SIZE) {        
        ...
        tile_a[local_row][local_col] =  select(0.0, a.val[global_a], ((k + local_col) < matrix_size) && (global_row < matrix_size));
        tile_b[local_row][local_col] =  select(0.0, b.val[global_b], ((k + local_row) < matrix_size) && (global_col < matrix_size));
    ...
    if(global_id.x < matrix_size && global_id.y < matrix_size) {
